...
test('the mapped set is the engine-expressible subset (D3)', () => {
  ...
  assert.equal(mapped.length, 17);   // WS-3 flipped 4 next-* twins; WS-4 adds 2 epic twins (bne-ship-clean, bne-e8-auto-no-self-merge)
  for (const id of ['add-fold-core', 'add-orphan', 'next-lane-complex-ship', 'next-auto-floor-pause', 'next-lane-doc-layer', 'next-auto-fork-resolve', 'bne-ship-clean', 'bne-e8-auto-no-self-merge']) assert.ok(mapped.includes(id), id);
  // next-lane-simple/complex stay P5: diff-driven classifyLane needs a real code diff the services-free sandbox can't produce.
  // bne-e8-pr-route stays P5: the single-PR route is host prose (WS-4 defers the gh-PR/worktree binding).
