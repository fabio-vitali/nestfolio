...
import type { PositionDifference, ComparisonDivergence } from '../services/comparison.service';
...
@Component({
      ...
      <div class="divergence-header">
        ...
        @if (divergence) {
          <div class="divergence-stats">
            <span class="stat">
              {{ i18n.t('dashboard.comparison.missedDecisions') }}: {{ divergence.missedDecisions }} / {{ divergence.totalDecisions }}
            </span>
            <span class="stat" [class.positive]="divergence.returnDifferenceCents > 0" [class.negative]="divergence.returnDifferenceCents < 0">
              {{ i18n.t('dashboard.comparison.valueDifference') }}: {{ divergence.returnDifferenceCents | currencyFormat }}
            </span>
          </div>
        }
      </div>
      ...
      >
        ...
        <ng-template pTemplate="body" let-pos>
          <tr>
            ...
            <td class="text-right" [class.positive]="pos.quantityDifference > 0" [class.negative]="pos.quantityDifference < 0">
  ...
  styles: [`
    ...
    .stat.positive { color: var(--green-500, #22c55e); }
    .stat.negative { color: var(--red-500, #ef4444); }
