...
test('new-investor-happy-path: onboarding → deposit → decision → logout', async ({
  ...
  //
  // The Confirm button is gated on status='AWAITING_CONFIRMATION', which is
  // pushed via the advisory-bff `onDecisionUpdate` WSS subscription when
  // USER_CONFIRMATION_REQUESTED arrives. advisory.confirm()'s 120s click
  // timeout polls for the button — covering the 45-90s compliance latency
  // without a manual reload. Validates the live WSS path end-to-end.
