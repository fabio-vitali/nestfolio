...
test('exit 1 when nestfolio-host index.html embeds amazonaws.com hosts', () => {
  // CSP was removed from the project (T12). index.html no longer contains any
  // amazonaws.com hosts. This test confirms the gate correctly flags such a file
  // if one were accidentally introduced.
  const root = makeTree({
    'apps/nestfolio-host/src/index.html': "<meta http-equiv=\"Content-Security-Policy\" content=\"connect-src 'self' https://cognito-idp.us-east-1.amazonaws.com https://cognito-identity.us-east-1.amazonaws.com\" />",
    'apps/nestfolio-host/src/main.ts': "console.log('clean');",
  });
  try {
    const r = runGate(root);
    assert.equal(r.status, 1, `expected gate to flag amazonaws.com in index.html, got status=${r.status}\nstderr: ${r.stderr}\nstdout: ${r.stdout}`);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
