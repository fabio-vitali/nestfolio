...
test('exit 0 when nestfolio-host generated index.html embeds Cognito hosts', () => {
  // The shell's apps/nestfolio-host/src/index.html is regenerated by the
  // prepare-index Nx target from index.html.tmpl + csp.txt. csp.txt admits
  // cognito-idp / cognito-identity *.amazonaws.com hosts as charter
  // exceptions, so the regenerated file inherits them. The gate excludes
  // that exact relative path; this test asserts that exclusion holds.
  const root = makeTree({
  ...
  try {
    const r = runGate(root);
    assert.equal(r.status, 0, `expected clean (regenerated index.html exclusion), got status=${r.status}\nstderr: ${r.stderr}\nstdout: ${r.stdout}`);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
