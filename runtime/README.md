...
| `scopeGate({activeItem, diffPaths})` · `activePartition(items)` | The §9.2 invariant: the working-tree diff belongs to the single active executable and every changed path is in its scope. The active law: ≤1 active non-epic + ≤1 active epic, zero legal. Ships a **self-resolving CLI** (`scope-gate.mjs [--single-active]`) that reads `docs/backlog` itself — the starter checks invoke it verbatim. |
