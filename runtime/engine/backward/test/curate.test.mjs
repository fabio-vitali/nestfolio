...
test('CU2 §11.2-3 sync guard-fail + human keep (default) → kind kept, no state change', () =>
  withTmpContent(async (dirs) => {
    seed(dirs.lessonsDir);
    const r = await runCurate({ guard: guard(), trigger: 'ship-gate-blocking', finding: finding('drift'), ask: async (d) => ({ decisionId: d.id, value: 'keep' }), journal: inMemoryJournal(), checksDir: dirs.checksDir, dossierRoot: dirs.lessonsDir });
...
test('CU3 §11.2-4 --auto at a curate floor → kind paused (lowering a guard is a hard-floor act)', () =>
  withTmpContent(async (dirs) => {
    seed(dirs.lessonsDir);
    const r = await runCurate({ guard: guard(), trigger: 'ship-gate-blocking', finding: finding('drift'), journal: inMemoryJournal(), checksDir: dirs.checksDir, dossierRoot: dirs.lessonsDir });
