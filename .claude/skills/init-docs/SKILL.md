...
Nuclear rebuild — regenerates ALL derived artifacts from code in 4 phases. Uses per-service and per-flow background agents to keep the main context clean.
...
- [ ] 2. **Dispatch one background agent per service** using `Agent` tool with `run_in_background: true`.
  ...
  **Sub-agent prompt template** (replace `{domain}` and `{service}`):
  ...
  2. services/{domain}/{service}/src/service.stack.ts — extract all constructs:
     ...
     - Orchestration: state machine names, triggers
     - Facade: GraphQL endpoints if present
     - AgentRuntime: Bedrock agent if present
  3. services/{domain}/{service}/src/handlers/ — list handler files
  4. services/{domain}/{service}/src/domain/events.ts — event constants
  ...
  - CDC: DynamoDB Streams → {service}-egress (Lambda)
    Emits: {events}
  ## Orchestration / Step Functions / Facade / AgentRuntime
  [Only sections that exist]
  ## Dependencies
  - libs: {list}

  Rules: No prose. Structured facts only. Omit empty sections. Use actual event names from code.
  ...
  CHECKS: [pass|fail|warn] per check (1-5)
...
- [ ] 4. **Dispatch one background agent per flow spec** with `run_in_background: true`.
...
- [ ] 7. **Dispatch a single cross-domain verification agent** with `run_in_background: true`:
  ...
  SKILL_FRESHNESS: N/22 skills have valid paths, list broken refs
  ```
