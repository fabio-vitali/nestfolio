...
The **member loop (E4) + epic-pre-done batch (E6) + merge floor** are driven by the runtime orchestrator:
run `node runtime/adapters/claude-code/run-epic.mjs <epic-id>`. The steps below (E4–E6) describe what the
orchestrator does at each park and the operator's fulfil actions. The adapter wraps the live spine
(`runtime/engine/loop/orchestrator.mjs`)
...
audit, E8 single PR + cleanup + postflight) remain the orchestrator's prose; epics still
