...
1. A **non-epic** item with `status: active` → resume it (the standalone workstream, or — if an epic is the active delivery epic — its in-flight member; either way you resume the single active workstream).
2. Else the **top-ranked QUEUED item** (lowest `rank`). Read `docs/backlog/<id>.md`; **if it is `type: epic`, redirect** — tell the user to run `/backlog-next-epic <id>` (epics are orchestrated there) and stop. Otherwise proceed by status.
...
**With `<id>` argument (`/backlog-next <id>`).** The argument overrides the rank pick. Locate `docs/backlog/<id>.md`. **If it is `type: epic`, stop and redirect to `/backlog-next-epic <id>`** (epics are not worked here — see "When to invoke"). Otherwise dispatch by status:
...
```bash
node .claude/skills/backlog-next/postflight.mjs --lane=<doc-layer|simple|complex|epic-member> [--branch=<feat-branch>]
```
...
Everything else (Step 1b effort marker, Step 2 verify references, Step 5 downstream routing, Steps 6.1–6.3, 6.5–6.6) runs unchanged.
