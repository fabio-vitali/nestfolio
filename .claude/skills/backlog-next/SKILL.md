...
Two sanctioned entry points: a user typing `/backlog-next` (standalone), **or** the `/backlog-next-epic` orchestrator driving this skill in epic-member mode. This skill is deliberately **NOT** `disable-model-invocation` (unlike `/backlog-next-epic`) so the orchestrator can drive it. **Standalone**, this SKILL.md loads inline in the session that invoked it. **In epic-member mode (Tier-2), the orchestrator dispatches this skill inside a dedicated `epic-member-worker` subagent** so the member's heavy work (reads, greps, test logs, debug loops) stays out of the orchestrator's context — the seam is the `Agent`-dispatch + `SendMessage` payload contract (see `backlog-next-epic` E4 / §B-§J). This **supersedes** the old "inline into the orchestrator's own context (not a detached subagent)" model that the E4.5 checkpoint and this now-shipped Tier-2 item reasoned about — that per-member `/clear` checkpoint is now only the dormant Tier-1 fallback. Outside those two paths, do not auto-fire it: the Step-1 active-in-flight guard + the epic-member guard below are the runtime backstops against a stray invocation.
...
**Epic-member mode (invoked by `/backlog-next-epic`).** When the orchestrator drives this skill, it **dispatches it as an `epic-member-worker` subagent** and passes a member `<id>` **plus an epic-member context signal** (the active epic + its branch/worktree + any pre-decided `(fork_key, chosen)` pairs). In that mode the worker runs the member *inside the already-active epic worktree, in its OWN context*, communicates with the orchestrator **only by `SendMessage`** (it has no `AskUserQuestion`), and hoists branch-level concerns to the epic. See § "Epic-member mode" below for the per-step deltas.
...
> **Epic-member guard.** A queued/active member of an *active* epic is a non-epic file, so the `type: epic` redirect alone won't catch it. Whenever a picked item (default rank-pick OR named `<id>`) has an `epic:` pointer to a `status: active` epic, **redirect to `/backlog-next-epic <epic-id>` and stop** — do not work it standalone. **Exception:** when *this* skill is invoked BY `/backlog-next-epic` in epic-member mode — i.e. dispatched as the `epic-member-worker` subagent with the epic context (active epic, branch/worktree, pre-decided `(fork_key, chosen)` pairs) — proceed with the member; that IS the intended path. The guard fires only for bare standalone `/backlog-next` calls.
...
- **Step 5 (Downstream routing).** Route to the same downstream skill as usual, with ONE critical change: `superpowers:executing-plans` and `superpowers:subagent-driven-development` end with an **unconditional handoff to `superpowers:finishing-a-development-branch`** (their "complete development" step). In epic-member mode you must **drive only their task-execution phase and STOP before that finishing handoff** — the member is "done" at commit-on-branch + green per-member integration tests. Do NOT let the downstream skill open a PR, merge, or clean the branch; **return control to `/backlog-next-epic` by `SendMessage`-ing your terminal `MEMBER-SUMMARY` payload to `main`** (do not just end your turn — a teammate's plain output is invisible to the orchestrator). This is what preserves the one-branch/one-PR invariant: the finishing handoff is the orchestrator's job at epic close (E8), never the member's. (If a member routes to `superpowers:brainstorming` for a `type: design` slice, see the orchestrator's E5 — in `--auto` such members do not auto-resolve the brainstorming approval gate.)
- **Decision bubble-up + floor (you are the `epic-member-worker` subagent — you have NO `AskUserQuestion`).** You do **NOT** prompt the user. Surface every decision you cannot self-resolve as a `NEEDS-DECISION` payload **`SendMessage`d to `main`** (the orchestrator runs the AskUserQuestion and `SendMessage`s the ruling back — continue from where you paused). Treat any passed-in `(fork_key, chosen)` pair as **pre-decided**: do not re-ask it, and for an **override** ADOPT the imposed `chosen` value. A `type: design` brainstorming approval gate and any irreversible / outward-facing action (staging/prod ops, real-money/broker actions, `git push --force`, `git branch -D`, destructive deletes, anything outside `dev-*`) are **NEVER** self-resolved — always bubble them up (`reason: design-approval` / `floor:<which>`); default permission-prompting is NOT a backstop inside a subagent (it auto-proceeds), so this checklist + the blast-radius gate ARE the floor. In `--auto`, self-resolve genuinely **non-floor** forks (classify via `node .claude/skills/backlog-next-epic/detect-fork-blast-radius.mjs <symbol>` — exit 1 = shared surface = floor = bubble) by picking the most-reusable `(Recommended)` option, **AND append each to run-state at resolution time** (`runstate.mjs append-decision <epic-id>`, with a `fork_key` computed from `fork-key.mjs`); also report them in your `MEMBER-SUMMARY` `decisions[]` (informational). This **replaces** the old "pause via AskUserQuestion (a prose pause is a skill violation)" clause — in subagent mode you cannot prompt at all; the prose-pause prohibition now lives at the orchestrator's E5, where the AskUserQuestion actually happens. (Mirrors orchestrator §D/E5 — F-8.)
- **`status: blocked` vs `NEEDS-DECISION`.** Emit `status: blocked` (terminal) ONLY when **no user ruling within this member could unblock it** — a missing external precondition, or the member is mis-scoped/too-large and needs a split. If a user *choice* could unblock it, emit a `NEEDS-DECISION` instead. (`blocked` is not a member-execution fork, but it needs an epic-level disposition — the orchestrator routes it via §H.) **Prefix every git commit subject with `[<member-id>]`** so the epic PR body (E8) can attribute commits per member.
- **Step 6.1 (Regen derived docs).** Pass the **member-start HEAD** captured at Step 4 as the base — `node .claude/skills/backlog-next/detect-doc-derivation.mjs --base=<member-start-HEAD>` — so the detector reports only THIS member's source delta. Diffing the cumulative branch vs the default `origin/main` always reports `derivation=true` while any *earlier* member's source differs, even when that member already regenerated and committed its derived docs — the false "always-true-on-resume" signal that would re-run `generate-c4-diagrams` / `audit-service` / `validate-flow` every resume (F-2). (Standalone/non-epic runs keep the default `origin/main` base — there is no prior member, so the cumulative diff IS the member delta.)
...
- **Step 7 (Postflight).** Run `node .claude/skills/backlog-next/postflight.mjs --lane=epic-member --id=<id>` (checks 1–3 only). Then **return control to `/backlog-next-epic` by `SendMessage`-ing your terminal `MEMBER-SUMMARY` payload to `main`** — it parses the payload (`member-summary.mjs`) and advances to the next member or moves to the epic pre-done gate.
