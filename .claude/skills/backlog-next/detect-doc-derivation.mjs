 ...
 *
 * `--base=<sha>` is what makes this resume-aware. On an epic-member resume, pass
 * the MEMBER-START HEAD so the detector reports only the current member's source
 * delta — diffing the cumulative branch vs origin/main always reports
 * `derivation=true` while any earlier member's source differs, even when that
 * member already regenerated and committed its derived docs (the F-2 false
 * "always-true-on-resume" signal). See the /backlog-next Epic-member Step 6.1 delta.
 *
 * Exit codes:
...
/**
 * Pure classification of a changed-file list into derived-doc regeneration actions.
 *
 * Git access is injected via `baseExists` — a `(svcDir) => boolean` predicate that
 * reports whether `<svcDir>/project.json` existed on the diff base — so this
 * function is import-safe and unit-testable (mirrors
 * detect-deploy-needed.mjs:classifyChanges, which the sibling F-3 fix aligns it to).
 *
 * @param {string[]} changedFiles
 * @param {{ baseExists: (svcDir: string) => boolean }} opts
 * @returns {{
 *   derivation: boolean,
 *   actions: { action: string, files: string[] }[],
 *   affectedServices: string[],
 *   newServices: string[],
 *   flowSpecsTouched: string[],
 *   adapterChanges: string[],
 * }}
 */
export function classifyDerivation(changedFiles, { baseExists }) {
...
function main() {
  ...
  // Injected git-backed predicate: did `<svcDir>/project.json` exist on the base ref?
  const baseExists = (svcDir) => {
    try {
      execSync(`git cat-file -e ${base}:${svcDir}/project.json`, {
        cwd: REPO_ROOT,
  ...
  const result = classifyDerivation(changedFiles, { baseExists });
  ...
  if (asJson) {
    console.log(JSON.stringify({
      ...
      flowSpecsTouched: result.flowSpecsTouched,
      adapterChanges: result.adapterChanges,
      base,
      changedCount: changedFiles.length,
    }, null, 2));
  } else {
    console.log(`derivation=${result.derivation}`);
    console.log(`affectedServices=${[...result.affectedServices].sort().join(',')}`);
    if (result.newServices.length > 0) console.log(`newServices=${[...result.newServices].sort().join(',')}`);
    if (result.adapterChanges.length > 0) console.log(`adapterChanges=${[...result.adapterChanges].sort().join(',')}`);

    if (result.derivation) {
      console.log(`\nActions (${result.actions.length}):`);
      let i = 1;
      for (const { action, files } of result.actions) {
