...
Nestfolio's business capabilities are organized into three bounded contexts, consolidated from an initial eight-context decomposition. Each domain is an isolated sub-system with its own event bus, infrastructure, and deployment pipeline.
...
The initial analysis identified eight bounded contexts: identity, investor, advisory, compliance, execution, portfolio, notification, and operations. These are consolidated into three domains. Each merge is justified by shared actors, tight event loops, or common lifecycle triggers.

### Identity + Investor + Notification --> Investor Domain

- **Identity --> Investor**: Identity and investor share the same primary actor (the Investor) and the same lifecycle trigger (user registration creates an investor profile). Identity's vocabulary (JWT, PII, federation) is infrastructure-level authentication with no independent business features beyond Cognito hosted UI. The data ownership boundary between credentials and preferences is enforced by service separation within the domain (`investor-web` owns auth, `investor-bff` owns preferences), not by domain separation.
- **Notification --> Investor**: Notification has a distinct policy model (5 severity tiers, 3 timing modes) but no distinct actor -- it serves the same Investor. Its lifecycle is purely reactive to events from other domains. The notification pipeline (`investor-ctrl`) retains its own Step Functions workflow, DynamoDB state, and event subscriptions. The merge consolidates the notification inbox into `investor-bff`, which already owns the Settings screen where notification preferences live.

### Advisory + Compliance + Operations --> Advisory Domain

- **Compliance --> Advisory**: Compliance exists to validate advisory decisions. The `DECISION_PACKET_CREATED` to `DECISION_APPROVED/BLOCKED` exchange was the single highest-traffic cross-domain flow. Both share the decision lifecycle vocabulary. `compliance-ctrl` remains a separate deployable unit with its own state, ingress, and egress per regulatory requirements. The merge routes compliance events through `advisory-hub` instead of a separate `compliance-hub`, eliminating a forwarding hop.
- **Operations --> Advisory**: Operations monitors advisory (agent failures, shadow comparisons) and compliance (guardrail violations, suitability failures). These constituted 60% of `operations-ctrl`'s inbound event subscriptions. The merge eliminates four cross-domain forwarding rules. `operations-ctrl` retains its three independent Step Functions workflows.

### Execution + Portfolio --> Execution Domain

- Portfolio's core input is execution events (`ORDER_FILLED`, `PORTFOLIO_SNAPSHOT_IMPORTED`). The Dual Truth Model reconciliation loop -- `portfolio-ctrl` detects drift, pauses `execution-ctrl`, corrects projections, resumes `execution-ctrl` -- was the tightest cross-domain coupling in the original model. Housing both in the execution domain converts this into intra-domain coordination through the same EventBridge bus, eliminating the most latency-sensitive cross-domain forwarding.
...
| `compliance-ctrl` separate deployable | Own Step Functions, own DynamoDB table, own IAM role. Rehoused in advisory domain but deploys as its own CDK stack with independent ingress and egress |
...
1. **3 bounded contexts** (consolidated from 8). The 8-context decomposition created excessive inter-domain coupling (25+ forwarding routes, tight SAGA loops). The 3-domain model groups contexts that share the same primary actor, tight event loops, or common lifecycle triggers. Domain-internal service boundaries preserve the architectural guarantees that motivated the original split.
...
3. **3 Event Hubs** (reduced from 8). Cross-domain forwarding routes reduced from approximately 25 to 6 directional routes.
...
5. **Reconciliation is intra-domain.** The `portfolio-ctrl` to `execution-ctrl` SAGA (lock, pause, correct, resume) was the tightest cross-domain coupling. Both now coordinate through the same EventBridge bus.
