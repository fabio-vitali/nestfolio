...
| 03 | [Circuit Breaker](./circuit-breaker.md) | `advisory`, `execution`, `investor` | Advisory agent triggers circuit breaker to halt execution on risk events; broker-ctrl opens its own circuit breaker on adapter timeout and auto-heals via Step Functions |
