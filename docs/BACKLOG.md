...
- [runtime-item-schema-reconciliation](backlog/runtime-item-schema-reconciliation.md) [refactor] — Reconcile runtime/engine/schema/item.schema.ts with the real docs/backlog frontmatter (done_criteria→done_when, relax .strict/migrate legacy keys) and wire validateItem into the read path — today ItemSchema has no production importer. `[epic:runtime-operationalization · core]`
