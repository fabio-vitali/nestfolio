...
rollup: core 17/20 done · captured 2/12 done
- core · queued · [runtime-replatform-next-epic](backlog/runtime-replatform-next-epic.md)
...
4. [runtime-replatform-next-epic](backlog/runtime-replatform-next-epic.md) [refactor] — P5 WS-4 (spec §8, thin & last): re-platform backlog-next-epic onto runOrchestrator. Build the run-epic.mjs CLI driver that does not exist today (the orchestrator spine has no adapter), wrapping the live spine + member-selection/rule-11/e2e-freshness. Deferred (epics are drained standalone per D1): the gh-PR-state probe and worktree-ops binding stay host-side. Promoted 2026-07-07: trigger fired — runtime-replatform-prereqs (2026-07-06) + add/lint/next (2026-07-07) all shipped. `[epic:runtime-operationalization · core]`
