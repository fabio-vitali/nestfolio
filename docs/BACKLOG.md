...
### `[design]` Collapse InvestorProfile to single-row CQRS BFF read model — SHIPPED 2026-05-04

**Adopted to ACTIVE:** 2026-05-03 (pivoted from "Onboarding fan-out 3 → 1" — see Recently Shipped table). **Shipped:** 2026-05-04 across phases 1-9 on `main` (commits `4d4c5679` spec → `e60caf76` Phase-9 SF wiring fix). All 6 services deployed to dev; integration tests green for investor-bff (17/17), dashboard-bff (17/17), compliance-ctrl (5/7 — 2 pre-existing fixture flakes filed); Playwright 5-run gate scored 2/5 full journeys + 5/5 onboarding completions (3/5 timeout at L1 user-confirm UX, filed). Spec: `docs/superpowers/specs/2026-05-03-investor-profile-collapse-design.md`. Plan: `docs/superpowers/plans/2026-05-03-investor-profile-collapse-plan.md`.
...
- **`compliance-ctrl` integration tests "REVOKED gate" + "L1 authority BALANCED" flaky** — 5/7 pass; the 2 stateful tests at `services/advisory/compliance-ctrl/test/integration/compliance-ctrl.integration.test.ts:290` race on out-of-order lambda invocations of prior tests' INVESTOR_PROFILE_* events (queued from earlier `it()` blocks land mid-sequence and overwrite the row). Per-test `userId` isolation + try/catch resilience already shipped 2026-05-04 (commit `dc905655`); first-attempt still flakes when the SQS Lambda reorders. Unit tests for `mandate-validator` + `rule-engine` + the projection paths all pass — not a Phase 9 regression. Cheapest fix likely: drain trap before each test's RECOMMENDATION_PROPOSED to clear stale events, or split into separate jest files for true SQS isolation. Promote if it blocks a deploy.
