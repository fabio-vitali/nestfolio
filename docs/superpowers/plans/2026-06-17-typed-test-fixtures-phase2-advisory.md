...
- **Bug triage (spec §7).** Each compiler-surfaced co-wrong fixture is **(a) fixture-only** → fix the fixture; or **(b) latent contract bug** (the fixture hid a real producer/consumer mismatch) → file via the `backlog-add` skill, do NOT fix production here. **Log the running (a)/(b) count** — no silent truncation.
  - **APPROVED EXCEPTION (2026-06-17, user-sanctioned, Task 1 only):** Task 1 completed `DecisionPacketSchema` with the 3 optional fields `taskToken`/`confirmedAt`/`rejectedAt` it was missing (the row carries them; `publisher-schemas.ts` was stripping them from the CDC subject → confirm/reject flow silently broken). This is an approved production contract change that rides along in Phase 2; the finding is filed (`dwc-decision-packet-schema-missing-optional-fields`) and its runtime validation (confirm/reject e2e on deployed dev) is owed to `typed-test-fixtures-consolidated-integration-e2e-verify`. This exception does NOT generalize — every other (b) finding still follows file-don't-fix unless separately approved.
