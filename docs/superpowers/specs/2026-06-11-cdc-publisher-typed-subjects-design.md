...
- The existing `transform?` seam is **removed** (no service uses it; `schemas` supersedes it —
  [[no-deprecation]]).
- `buildEntry` changes its one subject line:

  ```ts
  // before: subject: transform ? transform(record, eventType) : record
  // after:
  const schema = schemas[record.__typename];
  const subject = schema.parse(record);                       // DRY; throws on drift
  // detail.subject = subject
  // detail.previousSubject = emission.previousSubject ? schema.parse(emission.previousSubject) : undefined
  ```
...
**Home rule is trivially satisfied:** a publisher emits its *own* service's rows, so it imports
its *own* `@nestfolio/<svc>/contracts` — always intra-service. **WS-2 has no cross-domain
imports and therefore no project-cycle risk** (unlike consumers in WS-3).

### 3. Registry completeness guard

Forgetting a schema for a mapped row type must fail loudly, never silently emit untyped. Two layers:

- **Per-service unit test (primary, pre-deploy):** a shared helper asserts every `__typename`
  in the service's CDK `eventTypes` map has a key in the handler's `schemas` map
  (`Object.keys(eventTypes) ⊆ Object.keys(schemas)`). Cross-checks the two declarations that
  live in different files.
- **Cold-start assertion (defense-in-depth):** at handler init, derive the distinct
  `__typename` set from the `EVENT_TYPE_MAP` env var keys (`${__typename}:${ACTION}`) and assert
  each has a registered schema; throw at init otherwise.
...
- **Unit (comprehensive — every `__typename`, fast, no deploy):** drive
  `changeDataCapture({ schemas })` per service with a realistic row fixture and assert:
  (a) the emitted entry's `subject` **strict-equals** `Schema.parse(row)` and carries **no
  envelope keys** (`pk`/`sk`/`__typename`/identity absent); (b) a drifted row throws
  `NotRetryableError`; (c) the registry-completeness cross-check. Unit tests prove the DRY
...
1. **Shared-lib change** (`change-data-capture.ts` `schemas` param + DRY `buildEntry` +
   completeness guard + `NotRetryableError`) + its unit tests. Lands once.
2. **Ledger** (2 publishers): `ledger-ctrl`, `reconciliation-ctrl`. Smallest — proves the
   per-service pattern end-to-end before the wider domains. Run the ledger contract-emission gate.
3. **Investor** (3): `investor-ctrl`, `investor-bff`, `onboarding-bff`.
4. **Execution** (4): `execution-ctrl`, `broker-ctrl`, `broker-alpaca-adpt`, `broker-sim-adpt`
   (live-money path — validate against the real broker emissions).
5. **Advisory** (12): `investor-profile-ctrl`, `market-intelligence-ctrl`,
   `portfolio-engine-ctrl`, `advisory-narrative-ctrl`, `decision-workflow-ctrl`,
   `compliance-ctrl`, `advisory-bff`, `alpha-vantage-adpt`, `fred-adpt`, `marketwatch-adpt`,
   `sec-edgar-adpt`, `yahoo-finance-adpt`. Richest (`RegionContext`, bare-global, agent rows).
...
The plan enumerates each service's exact `eventTypes`-`__typename` → WS-1-schema mapping (from
each `service.stack.ts` Egress config + the service's `contracts.ts`).
...
4. **Missing/extra contract** — if a mapped `__typename` has no WS-1 contract, that is a WS-1 gap;
   correcting a contract that demonstrably does not match the real row is in scope (WS-2's
   verification purpose), but authoring net-new contracts for uncovered events is filed, not
   invented here (see Out of scope).
...
- Re-designing or net-new-authoring WS-1 contracts.
- Unrelated QUEUED items (`nx-affected-true-affected-resolver`,
  `dashboard-live-push-position-snapshots`, …).

## Done

Every CDC publisher imports its producer contracts and emits the **DRY parsed subject**; no
generic `Record` casts in the publisher path; a producer schema change breaks the publisher
build; the registry-completeness guard and `NotRetryableError` drift-to-DLQ are in place;
publisher unit tests + the executed contract-emission e2e gates (with representative DRY-wire
captures) are green against deployed dev.
