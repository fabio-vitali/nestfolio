...
- 2026-07-07 (runtime-replatform-next, WS-3): the epic-pre-done SHA-conditional batch (`orchestrator.mjs`'s inline block) is **extracted** into a shared ring-1 helper `preShipBatch({journal, runId, registry, changedScope, judge, headSha, contexts, cost_ceiling, on})` (`runtime/engine/loop/pre-ship-batch.mjs`). `runOrchestrator` delegates to it (behavior-preserving — `contexts:['audit','gate']`, `on:'epic-pre-done'`, `changedScope:['**/*']`), and `runWorker` gains a **new SHA-conditional pre-ship step between execute and ship-gate**. That step is **trigger-injected**, not lane-computing: the worker accepts an optional `preShipTrigger` (`{contexts, cost_ceiling, on}`) and optional `changedScope`; a **null** trigger skips the batch. The lane→trigger mapping (`classifyLane`/`laneToTrigger`) is **Nestfolio content computed by the adapter** (`run-next.mjs`), never imported by ring-1 — preserving the §-hard-constraint "ring-1 stays project-agnostic" (no engine→content edge; import-boundary test green). `contexts`/`cost_ceiling`/`on`/`changedScope` are all caller-supplied. Freshness stays the literal `'e2e'` key via `e2eIsFresh` (`journal.mjs:146`). Helper imports only `../lib/*`.
