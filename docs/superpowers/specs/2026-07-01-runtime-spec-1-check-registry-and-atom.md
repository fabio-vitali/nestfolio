...
- 2026-07-07 (runtime-replatform-next, WS-3): the epic-pre-done SHA-conditional batch (`orchestrator.mjs`'s inline block) is **extracted** into a shared ring-1 helper `preShipBatch({journal, runId, registry, changedScope, judge, headSha, contexts, cost_ceiling, on})` (`runtime/engine/loop/pre-ship-batch.mjs`). `runOrchestrator` delegates to it (behavior-preserving — `contexts:['audit','gate']`, `on:'epic-pre-done'`, `changedScope:['**/*']`), and `runWorker` gains a **new lane-gated step between execute and ship-gate** that calls it with `on:'item-pre-ship'` and the item's branch-delta scope (doc-layer lane skips it entirely). `contexts`/`cost_ceiling`/`on` are now caller-supplied (not hardcoded). Freshness stays the literal `'e2e'` key via `e2eIsFresh` (`journal.mjs:146`). Ring-1 imports unchanged (helper imports only `../lib/*`; import-boundary test green).
