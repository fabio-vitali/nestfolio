...
- No longer `providedIn: 'root'` — provided per route scope
- Constructor receives `APPSYNC_CONFIG` via injection
...
| `/ledger/orders` | (future) OrderHistoryComponent | Paginated ledger entries |
...
- `apps/nestfolio-host/public/assets/config.json`: key rename
...
- `comparison-card.component.ts`: routerLink changed from `['comparison']` → `['/ledger/simulation']`
...
|---------|---------|---------|
| `apollo-angular` | `13.x` | Angular integration for Apollo Client |
...
|---------|----------------|-----------------|
| `apollo-angular@13.x` | `@angular/core ^19 \|\| ^20 \|\| ^21` | `^21.2.0` |
| `apollo-angular@13.x` | `@apollo/client ^4.0.1` | `4.x` |
| `aws-appsync-auth-link@4.0.1` | `@apollo/client 4.x` | `4.x` |
| `aws-appsync-subscription-link@4.0.1` | `@apollo/client 4.x`, `rxjs ^7` | `4.x`, `^7` |

All peer dependencies align. The AppSync link packages are in maintenance mode (security/compat fixes only) but functional and compatible.
