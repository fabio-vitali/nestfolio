...
| Section 1B: SFN error name list might be incomplete | Resolved | Verified against `@aws-sdk/client-sfn@3.1011.0` `SendTaskSuccessCommand.d.ts:57-77`. The command throws exactly: `InvalidOutput`, `InvalidToken`, `KmsAccessDeniedException`, `KmsInvalidStateException`, `KmsThrottlingException`, `TaskDoesNotExist`, `TaskTimedOut`. The three names in the spec (`TaskTimedOut`, `InvalidToken`, `TaskDoesNotExist`) are all real. `TaskTimedOut` explicitly documents "task associated with the token has already been closed" — this is the exact duplicate-callback case. |
