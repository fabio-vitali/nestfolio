...
- DDB Streams CDC on MandateSnapshot:INSERT emits a NEW event `MANDATE_SNAPSHOT_CREATED` on advisoryBus (declarative `eventTypes` mapping on the existing Egress). _Originally landed as `ADVISORY_PIPELINE_READY`; renamed to producer-local-state-change naming in commit `095973d8` — see "Post-ship topology cleanup 2026-05-11" below._
- `TRIGGER_EVENT_TYPES` swap: `INVESTOR_PROFILE_CREATED` → `MANDATE_SNAPSHOT_CREATED`. `INVESTOR_PROFILE_UPDATED` stays for re-decisions.
...
- `dashboard-bff` + `investor-adpt`: as originally shipped, the advisory-domain trigger event was plumbed cross-domain (investor-adpt forwarded advisoryBus → investorBus; dashboard-bff ingress subscribed; advisory-status transform `TRIGGER_TYPES` Set swap so `pendingDecisionsCount` tracked the SF first-decision trigger). _Cleanup commit `aadca562` retargeted dashboard-bff to subscribe to `MANDATE_ISSUED` directly on investorBus, dropping the cross-domain forwarding entirely — see "Post-ship topology cleanup 2026-05-11" below._
...
**Read-your-write guarantee.** `MANDATE_SNAPSHOT_CREATED` is the CDC of THIS service's own MandateSnapshot:INSERT, so by the time the SF starts the row is already committed in the local table. INVESTOR_PROFILE_UPDATED + non-PROFILE triggers (DEPOSIT_DETECTED, ORDER_*) hit a long-since-materialised projection (onboarding's MANDATE_ISSUED ran during phase 6).
