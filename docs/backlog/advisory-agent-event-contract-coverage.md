...
topic_memory: []
validation_gate: "Shipped 2026-06-11 (worktree worktree-advisory-agent-event-contract-coverage; spec 2026-06-11-advisory-agent-event-contract-coverage-design.md, plan 2026-06-11-advisory-agent-event-contract-coverage.md). DONE criteria met: (1) all 6 consumer-having events have producer row-level zod contracts — PortfolioAgentCompletionSchema/PortfolioAgentFailureSchema (portfolio-engine-ctrl/contracts), NarrativeAgentCompletionSchema/NarrativeAgentFailureSchema + ExplanationGeneratedSchema (advisory-narrative-ctrl/contracts), AdvisoryStatusSchema (advisory-bff/contracts) — built from a NEW reusable AgentCompletionRowSchema/AgentFailureRowSchema generator in @nestfolio/agent-orchestrator (commit a0bbca17); (2) all 8 consumer-less telemetry events stop-emitted (removed from each service's CDK Egress eventTypes + publisher exemptTypenames): GOAL_INTERPRETATION_PRODUCED+RISK_EVALUATION_PRODUCED (IP), MARKET_SIGNAL_DETECTED (MI), PORTFOLIO_CONSTRUCTION_PROPOSED+REBALANCE_PLAN_PRODUCED (PE), AGENT_OUTPUT_CREATED/UPDATED (DWC), AUDIT_ARTIFACT_CREATED/UPDATED (compliance), USER_INTERACTION_CREATED/UPDATED (advisory-bff); (3) ALL 7 advisory-core publisher exemptTypenames drained to [] (grep-verified). Plus a Decision-4 surgical consumer fix: dashboard-bff/transforms/advisory-status.ts now reads tenantId from uow.event.context (the only consumer of the 6 newly-DRY events that read a stripped field; sfn-callback already had a ctx fallback; EXPLANATION_GENERATED has no subject consumer). VALIDATION: comprehensive unit layer green across agent-orchestrator + all 7 advisory-core services + dashboard-bff (PE 118, AN ~65, advisory-bff 48, IP 76, MI 75, DWC 151, compliance 81, dashboard-bff 54). `nx affected -t build,lint,test --base=origin/main` GREEN except 1 pre-existing UNRELATED failure (agent-orchestrator invoke-agentcore.test.ts phantom @smithy/util-stream dep, MODULE_NOT_FOUND on main too — filed agent-orchestrator-invoke-agentcore-smithy-phantom-dep). Deployed 8 services to dev sandbox clean (33 CFN COMPLETE, 0 FAILED). REAL-ROW validation (spec fallback, user-approved): PortfolioAgentCompletionSchema + AdvisoryStatusSchema confirmed against real deployed-dev rows; NarrativeAgentCompletionSchema (identical generator to the proven PE contract) + ExplanationGeneratedSchema (writer-code-confirmed) unit-validated but no real dev row available (AN table TTL-empty of AgentCompletion/ReasoningOutput). The full-cycle advisory-contract-emission e2e Block A (which now includes all 6 new real-row asserts, Task 9) is BLOCKED by the documented sandbox AgentCore maxVms flake — failed at beforeAll `withProfileSnapshot(): InvestorProfileSnapshot not materialised within 360s`; CloudWatch-confirmed NOT a regression (the IP snapshot row DID materialise >360s, IP egress publisher healthy with zero contract violations, all this workstream's changes are egress-side). The 6 e2e asserts will validate once the maxVms remediation lands (agentcore-maxvms-prod-quota-increase / contract-emission-dry-wire-reenable). Doc-derivation shipped: 8 service cards regenerated (commit 30ddc44c), advisory-cycle + market-data-ingestion flow specs updated, portfolio-rebalance.flow.yaml broad pre-existing staleness filed (portfolio-rebalance-flow-spec-precomputation-stale)."
---
