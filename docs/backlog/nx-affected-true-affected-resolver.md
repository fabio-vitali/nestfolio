...
references: []
out_of_scope:
  - "Type-only-diff → false `deploy=true`: a type-only change has zero runtime-artifact delta but still 'touches' the file, so it is affected under ANY graph resolver. That is a build-output (bundle/.js-hash) question tracked under cdk-bundle-staleness-deploy-integrity — NOT this resolver's axis (graph membership)."
  - "Upgrading nx to fix the over-report: proven not version-fixable (22.7.5 changelog has no fix; projectsAffectedByDependencyUpdates had no effect). We compute the affected set ourselves from the correct `nx graph` JSON instead."
