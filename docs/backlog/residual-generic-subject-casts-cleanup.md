...
> **DROPPED 2026-06-09.** The `opaqueSubject` idiom at this workstream’s core was the wrong
> answer — in a system where every event has a producer, every subject can be typed. Superseded
> by the typed-subject program: `typed-subject-producer-contracts` (WS-1) → `cdc-publisher-typed-subjects`
> (WS-2) → `consumer-parse-subject` (WS-3), strategy in
> `docs/superpowers/specs/2026-06-09-typed-subject-program-strategy.md`. Salvaged to main: the
> assemble-packet SF-state interface typing + the dead-`OrderLifecycleService` deletion. The
> execution-ctrl USER_CONFIRMED `decisionId` id-fallback is captured for WS-3.
