...
validation_gate: "CLOSED 2026-07-10 — the runtime is the project's LIVE enforcement + work-driver, all 7 done_when clauses demonstrated: (1) enforcement — diff-scoped commit gate live + ~34-surface migration complete (deterministic + judgment tiers, live judge binding) + golden gates in CI [runtime-make-it-fire, runtime-check-migration-completion, -judgment-tier, -exclusions-content-ring, -goldengates-ci]; (2) backward edge live-in-anger — mint through a real floor + curate-at-the-floor as the only sanctioned bypass, skip-hatch instrumented [runtime-backward-edge-live, runtime-redteam-hardening]; (3) item.schema reconciled + validated on read [runtime-item-schema-reconciliation]; (4) parity oracle green — FINAL sweep 17/17 dominant on live Opus 4.8 (2 runtime-strictly-better) + greenfield adoption e2e [runtime-regression-harness, runtime-replatform-soak-gate]; (5) work-driver re-platformed + soaked over 13 fallback-free runtime workstreams (≥5 floor) with 0 path-fallbacks [runtime-work-driver-replatform, -replatform-prereqs/-add/-lint/-next/-next-epic, -soak-gate]; (6) operator surface (view+executor) shipped [runtime-operational-surface]; (7) legacy work-driver RETIRED — flag-off prose bodies + strangler seams + RUNTIME_ENGINE flag + parity-oracle/benchmark comparator all deleted; the runtime paths are the ONLY paths [runtime-legacy-retirement, merge acd44767]. All 27 core members terminal. Captured audit (12 open captured members, closure-predicate test): none load-bearing → none promoted to core; 2 dropped as moot (subjects deleted in P6: benchmark-backlog-skill-cost-figures-stale, bef-scenario-tags-reusable-suite), 1 shipped as resolved (runtime-guide-path-to-live-section-stale, GUIDE.md rewritten in P6), 9 genuinely-orthogonal spun out to runtime-operationalization-leftovers. backlog-lint 11/11 green post-close."
---
