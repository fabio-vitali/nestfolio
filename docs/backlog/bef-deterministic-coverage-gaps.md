...
out_of_scope:
  - "bef judge EVIDENCE-completeness bugs (e.g. bef-judge-blind-to-subworktree-diff in backlog-eval-corpus-hardening-leftovers) — that is the judge not SEEING the full diff, a distinct cause from deterministic-teeth limits on model-behavior invariants."
