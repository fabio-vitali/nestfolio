...
export class DashboardRepository extends TableRepository {
  ...
  @log()
  async getPositionSnapshots(tenantId: string): Promise<Record<string, unknown>[]> {
    const pk = dashboardPk(tenantId);
  ...
  @log()
  async getRecentActivity(tenantId: string, limit: number = 20): Promise<Record<string, unknown>[]> {
    const pk = dashboardPk(tenantId);
