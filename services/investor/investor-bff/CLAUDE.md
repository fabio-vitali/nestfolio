...
- Mandate: MANDATE_ISSUED, MANDATE_REAFFIRMED, MANDATE_REVOKED, OPERATING_MODE_CHANGED
...
- AppSync GraphQL API with JS resolvers (discoverJsResolvers)
  ...
  - Mutation: updateGoal(input), updateOperatingMode, updateRiskProfile(toleranceIdx, experienceIdx) (`.fn.ts`, esbuild-bundled — imports `computeRiskProfile`), confirmGoLive (returns InvestorProfile), revokeMandate (returns Mandate), initiateDeposit, requestWithdrawal, requestAccountClosure (noneDataSource), updateFeatureFlag (@aws_iam), markNotificationRead
  ...
  - Pipeline extraSteps: get-profile-mandate.fn.js appended to getProfile (fetches Mandate sibling row and merges into response); get-profile.fn.js readback appended to updateOperatingMode, updateRiskProfile, confirmGoLive
- confirmGoLive resolver issues a single atomic `TransactWriteItems` with 3 items (preconditioned executionMode='simulation' + Mandate status='ACTIVE'): write-once ExecutionModeChange audit row (→ EXECUTION_MODE_CHANGED → broker-ctrl), flip InvestorProfile executionMode→live + bump `__version` (→ INVESTOR_PROFILE_UPDATED → dashboard-bff InvestorSnapshot live badge), re-affirm Mandate effectiveDate + bump `__version` (→ MANDATE_REAFFIRMED → compliance-ctrl MandateSnapshot). Idempotent: a second attempt fails cleanly with InvalidState.
- updateRiskProfile resolver (`.fn.ts`, esbuild-bundled via the Facade `.fn.ts` path) recomputes riskProfile from (toleranceIdx, experienceIdx) using the owned `computeRiskProfile` algorithm (single source of truth) and writes it to the composite InvestorProfile row (bumps `__version`).
...
- InvestorBffEventTypes: ACCOUNT_CLOSED, ACCOUNT_CLOSURE_REQUESTED, BROKER_AUTHORIZATION_REVOKED, BROKER_CIRCUIT_CLOSED, BROKER_CIRCUIT_OPEN, DEPOSIT_DETECTED, DEPOSIT_FAILED, DEPOSIT_INITIATED, DEPOSIT_REQUESTED, DEPOSIT_SETTLED, EXECUTION_MODE_CHANGE_UPDATED, EXECUTION_MODE_CHANGED, GOAL_UPDATED, INVESTOR_PROFILE_CREATED, INVESTOR_PROFILE_UPDATED, MANDATE_ISSUED, MANDATE_REAFFIRMED, MANDATE_REVOKED, NOTIFICATION_READ, ONBOARDING_ANSWER_RECORDED, ONBOARDING_COMPLETED, OPERATING_MODE_CHANGED, PII_REMOVED, TENANT_ANONYMIZED, USER_AUTHENTICATED, USER_DELETION_REQUESTED, USER_REGISTERED, USER_SESSION_EXPIRED, WITHDRAWAL_FAILED, WITHDRAWAL_INITIATED, WITHDRAWAL_REQUESTED, WITHDRAWAL_SETTLED
