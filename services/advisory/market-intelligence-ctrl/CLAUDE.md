...
- MarketSnapshotRefreshTickSchema / MarketSnapshotRefreshTick — subject carried on MARKET_SNAPSHOT_REFRESH_TICK (self-tick from scheduled-emitter.ts). Fully DRY: `z.object({})` — no subject fields. `region` is identity and travels in the RegionContext; the slow-tier handler reads it from `ctx.region` (was `subject.region` until the 2026-06-26 dead-code sweep aligned the tick with MarketSnapshotSchema's subject→context migration).
