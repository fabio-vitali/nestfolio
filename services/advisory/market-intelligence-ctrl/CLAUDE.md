...
- MarketSnapshotRefreshTickSchema / MarketSnapshotRefreshTick — subject carried on MARKET_SNAPSHOT_REFRESH_TICK (self-tick from scheduled-emitter.ts). Fields: region (string). The `region` field is the only payload — identity context travels in RegionContext.
