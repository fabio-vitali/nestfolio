...
## State
- DynamoDB table (streams enabled). Holds DecisionPacket + AgentOutput + MandateSnapshot rows. WorkflowTrigger DDB row REMOVED post-collapse — direct EB → SF eliminated the trigger-aggregator hop. MandateSnapshot is a service-private projection (added 2026-05-10) keyed pk=`MandateSnapshot#{tenantId}#{userId}`, sk='MandateSnapshot' carrying operatingMode + level + status — read by the SF via Direct DDB GetItem.

## Ingress (CallbackIngress + MandateProjectorIngress)
...
- CDC: DynamoDB Streams → decision-workflow-ctrl-egress (Lambda)
  ...
  - MandateSnapshot → ADVISORY_PIPELINE_READY (insert only — operatingMode changes do NOT re-trigger first decision)
...
- DecisionStateMachine: Step Functions state machine (72h timeout)
  ...
  After UnpackTriggerEnvelope, the SF runs LookupMandateSnapshot (Task: arn:aws:states:::dynamodb:getItem) + SetInvestorProfile (Pass) to resolve operatingMode from the MandateSnapshot projection. This is single-path — every trigger reaches it; no payload-vs-projection branching. SF role granted dynamodb:GetItem on the local State table.
...
- DecisionWorkflowEventTypes: DECISION_PACKET_CREATED, DECISION_PACKET_UPDATED, ANALYZE_INVESTOR_PROFILE, ANALYZE_MARKET, CONSTRUCT_PORTFOLIO, GENERATE_NARRATIVE, RECOMMENDATION_PROPOSED, USER_CONFIRMATION_REQUESTED, DECISION_FEEDBACK, DECISION_WORKFLOW_FAILED, AGENT_OUTPUT_CREATED, AGENT_OUTPUT_UPDATED, ADVISORY_PIPELINE_READY
- TRIGGER_EVENT_TYPES (7): ADVISORY_PIPELINE_READY, INVESTOR_PROFILE_UPDATED, PORTFOLIO_DRIFT_DETECTED, ORDER_FILLED, ORDER_REJECTED, ORDER_CANCELLED, DEPOSIT_DETECTED
...
- mandate-projector.test.ts
- mandate-snapshot.repository.test.ts
