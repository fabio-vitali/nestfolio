...
- AppSync GraphQL API (Cognito auth via investor user pool SSM; IAM auth also enabled)
  ...
  Pipeline steps:
  - confirmDecision: preStep + extraStep → get-decision-readback.fn.js (reads existing DecisionReadModel to lift taskToken + proposedTrades); confirm-decision.fn.js writes ONLY UserConfirmation intent row (PutItem) — no DecisionReadModel write. Stamps proposedTrades (from the readback row, when present) onto the UserConfirmation row for order-execution money-path WS-1.
...
- UserConfirmationSchema / UserConfirmation — `UserConfirmation#${autoId}` intent row (JS resolver PutItem), CDC-emitted USER_CONFIRMED. Fields: decisionId, confirmedAt, confirmedBy, timestamp, taskToken?, proposedTrades?:unknown[] (opaque, optional — stamped from the readback DecisionReadModel for order-execution money-path WS-1; typed by the execution-ctrl consumer in WS-2).
