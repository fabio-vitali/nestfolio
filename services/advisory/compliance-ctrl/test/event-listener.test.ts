...
describe('event-listener handler', () => {
  ...
  describe('mandate events', () => {
    ...
    it('should skip unknown mandate event types gracefully via default case', async () => {
      // OPERATING_MODE_CHANGED is handled (just logs), so use it
      const sqsEvent = buildSqsEvent([
        {
          messageId: 'msg-opmode',
          body: {
            detail: {
              id: 'evt-opmode',
              type: 'OPERATING_MODE_CHANGED',
              timestamp: '2025-01-01T00:00:00.000Z',
              subject: { tenantId: 't-1', mode: 'AUTONOMOUS' },
              context: { tenantId: 't-1' },
            },
          },
        },
    ...
    it('should throw on MANDATE_GRANTED with missing required fields', async () => {
      ...
      const sqsEvent = buildSqsEvent([
        {
          messageId: 'msg-missing-mandate',
          body: {
            detail: {
              id: 'evt-missing-mandate',
              type: 'MANDATE_GRANTED',
              timestamp: '2025-01-01T00:00:00.000Z',
              subject: {
                tenantId: 't-1',
      ...
      const result = await handler(sqsEvent);
      expect(result.batchItemFailures).toHaveLength(1);
      expect(result.batchItemFailures[0].itemIdentifier).toBe('msg-missing-mandate');
    });
  });
});
